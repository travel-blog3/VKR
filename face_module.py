...
def start(): # загрузка путей к каскадам хаара
...
def process_and_encode(images):
    ...
    for image_path in os.listdir(images):
        image = cv2.imread('./images/' + image_path) # конвертирование в RGB
        ...
        if image_path == "ales_17.jpg":
            name = "aleksandr"
        elif image_path == "pasha_18.jpg":
            name = "pasha"
        else:
            name = image_path[:4]
        ...
        for (x,y,w,h) in faces:
            ...
            # which can detect open and closed eyes                
                ...
                for (ex,ey,ew,eh) in right_eye:
                    color = (0,255,0)
                    pred = pred(right_face[ey:ey+eh,ex:ex+ew],model)
                    if pred == 'closed':
                        eye_status='0'
                        color = (0,0,255)
                ...
                for (ex,ey,ew,eh) in left_eye:
                    color = (0,255,0)
                    pred = pred(left_face[ey:ey+eh,ex:ex+ew],model)
                    if pred == 'closed':
                        eye_status='0'
                        color = (0,0,255)
...
if __name__ == "__main__":
    (model, face_detector, open_eyes_detector,left_eye_detector,right_eye_detector, video_capture, images) = start()
    images = './images'
